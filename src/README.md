...
Since we can only send integers between 0-255 in a single byte, we use a mapping system to send floating point values using bytes. 0 to 49 sets a key, and a byte of 50 to 250 means the value of that key. When ESP receives a byte between 0 to 49, it saves that key and waits for the value to arrive.
...
1. By accumulating this movement vector, we can find the realtime position of the robot.
